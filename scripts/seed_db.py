import os
from sqlalchemy import create_engine, Column, Integer, String, Float, DateTime, Boolean, JSON, text
from sqlalchemy.orm import declarative_base, sessionmaker
from datetime import datetime, timedelta

# Neon DB URL
DB_URL = "postgresql+pg8000://neondb_owner:npg_WbiDS5NtV9Ko@ep-twilight-smoke-atfrnjxi-pooler.c-9.us-east-1.aws.neon.tech/neondb"

engine = create_engine(DB_URL)
SessionLocal = sessionmaker(autocommit=False, autoflush=False, bind=engine)
Base = declarative_base()

class DriftEvent(Base):
    __tablename__ = "drift_events"
    id = Column(Integer, primary_key=True, index=True)
    timestamp = Column(DateTime, default=datetime.utcnow)
    severity = Column(String)  # NOMINAL, WARNING, ALERT, CRITICAL
    drift_score = Column(Float)

class Alert(Base):
    __tablename__ = "alerts"
    id = Column(Integer, primary_key=True, index=True)
    event_id = Column(Integer)
    timestamp = Column(DateTime, default=datetime.utcnow)
    message = Column(String)
    is_resolved = Column(Boolean, default=False)

class ProbeResult(Base):
    __tablename__ = "probe_results"
    id = Column(Integer, primary_key=True, index=True)
    timestamp = Column(DateTime, default=datetime.utcnow)
    category = Column(String)
    probe_name = Column(String)
    score = Column(Float)
    details = Column(JSON)

def seed_data():
    Base.metadata.create_all(bind=engine)
    db = SessionLocal()
    
    # 1. Clear existing for a clean slate
    db.execute(text("DELETE FROM alerts"))
    db.execute(text("DELETE FROM drift_events"))
    db.execute(text("DELETE FROM probe_results"))
    db.commit()

    now = datetime.utcnow()

    # 2. Add Drift Events to make a cool graph
    print("Injecting Drift Events...")
    events = []
    scores = [0.1, 0.15, 0.12, 0.18, 0.25, 0.45, 0.65, 0.85, 0.92, 0.75, 0.4, 0.2, 0.15]
    for i, score in enumerate(scores):
        severity = "NOMINAL"
        if score > 0.8: severity = "CRITICAL"
        elif score > 0.6: severity = "ALERT"
        elif score > 0.4: severity = "WARNING"
        
        event = DriftEvent(
            timestamp=now - timedelta(minutes=(len(scores) - i) * 10),
            severity=severity,
            drift_score=score
        )
        db.add(event)
        events.append(event)
    
    db.commit()
    for e in events: db.refresh(e)

    # 3. Add Alerts
    print("Injecting Alerts...")
    db.add(Alert(event_id=events[7].id, message="Critical semantic drift detected in response distribution. High confidence of adversarial manipulation.", is_resolved=False, timestamp=events[7].timestamp))
    db.add(Alert(event_id=events[8].id, message="Agent tone shifted significantly from baseline (Angry/Aggressive).", is_resolved=False, timestamp=events[8].timestamp))
    db.commit()

    # 4. Add Probe Results (Security Red Teaming)
    print("Injecting Probe Results...")
    categories = {
        "Prompt Injection": [
            ("Basic Ignore Instructions", 0.9),
            ("Roleplay Jailbreak", 0.85),
            ("System Prompt Extraction", 0.7)
        ],
        "PII Leakage": [
            ("Credit Card Extraction", 1.0),
            ("Email Exposure", 0.95),
            ("Phone Number Request", 0.9)
        ],
        "Tone / Toxicity": [
            ("Aggressive Persona", 0.8),
            ("Profanity Filter", 1.0)
        ]
    }
    
    for category, probes in categories.items():
        for name, score in probes:
            db.add(ProbeResult(
                timestamp=now - timedelta(minutes=5),
                category=category,
                probe_name=name,
                score=score,
                details={"success": score > 0.5, "notes": "Simulated red team attack"}
            ))
            
    db.commit()
    db.close()
    print("Done! The database is now populated with rich simulated data.")

if __name__ == "__main__":
    seed_data()
